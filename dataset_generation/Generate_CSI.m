% Author: Dingzhong Wang
% Demo CSI generation code
clc;clear;close all;
s = qd_simulation_parameters;
s.center_frequency = 3.5e9; %center frequency，5G NR的新FDD频段
...
ElcTltAgl_BS = 7;
Hspc_Tx_BS = 0.5s.wavelength; % Horizontal array element spacing
Vspc_Tx_BS = 0.5s.wavelength; % Vertical array element spacing

BSAntArray = qd_arrayant.generate('3gpp-mmw',M_BS,N_BS,...
s.center_frequency,2,ElcTltAgl_BS,...
Vspc_Tx_BS/s.wavelength,Mg_BS,Ng_BS,...
Vspc_Tx_BS/s.wavelengthM_BS,Hspc_Tx_BS/s.wavelengthN_BS);
...
Speed=5.15:0.15:140; %[km/h]
H_U_his = zeros(900,10,16,64,4,4,2);
H_U_pre = zeros(900,10,4,64,4,4,2);
H_D_pre = zeros(900,10,4,64,4,4,2);
...
for ii=1:UENum
    h=BS2UE_channel(ii).fr(23040e3,128);  % OFDM channel generation，频域CSI，128频点
    h=reshape(h,2,4,4,128,20); %整理维度，time(20) × freq(128) × Nv(4) × Nh(4) × pol(2)
    h=permute(h,[5,4,3,2,1]);
    H_U_his(iter_Speed,ii,:,:,:,:,:)=h(1:16,1:64,:,:,:);  % historical uplink CSI
    H_U_pre(iter_Speed,ii,:,:,:,:,:)=h(17:20,1:64,:,:,:); % future uplink CSI
    H_D_pre(iter_Speed,ii,:,:,:,:,:)=h(17:20,65:128,:,:,:); % future downlink CSI
end