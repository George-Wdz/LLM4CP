...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train LSTM baseline on LLM4CP data")
    # data paths
    ...
    parser.add_argument('--pretrained-path', type=str, default=None,
                        help='Optional LSTM checkpoint to resume from (expects torch.save(model, path) format)')
    ...
    parser.add_argument('--layers', type=int, default=4,
                        help='Number of stacked LSTM layers (baseline uses 4)')
...
def main() -> None:
    ...
    if prev_len != 16 or pred_len != 4:
        raise ValueError(f"LSTM baseline expects prev_len=16 and pred_len=4, but got prev_len={prev_len}, pred_len={pred_len}")

    input_size = args.input_size or feature_dim
    model = LSTM(features=feature_dim, input_size=input_size, hidden_size=args.hidden_size, num_layers=args.layers)
